...
function Register() {
  ...
  const signup = async () => {
    ...
    try {
      await axios.post("http://localhost:8089/users", { username, password, phone_number: phoneNumber });
      await axios.post("http://localhost:8089/users/role/addtouser", { username:username, roleName: "user" });
  ...
  return (
    <Box sx={{
      ...
      <Typography variant="body1" sx={{ textAlign: 'center', mb: 2 }}>
        Bạn đã có tài khoản? <Link style={{textDecoration:'none'}} component={RouterLink} to="/login">Đăng nhập</Link>
      </Typography>
      <Button
        fullWidth
        variant="contained"
